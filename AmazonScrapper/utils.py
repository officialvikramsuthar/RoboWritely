...
class AmazonScrapperPrompats(CreateGptBlog):
    def get_intro_prompt(self):
        intro_prompt = f""" Get Introduction for  "{self.title}" """

        return intro_prompt

    def get_meta_descriptions_prompt(self):
        meta_description_prompt = f""" Get meta description for the keyword "{self.title}" """

        return meta_description_prompt

    def get_product_para(self, name, description, price, reviews="", rating=""):
        prompt = f"""Get Product Section for name :{name}, description : {description}, price : {price}, reviews: {reviews}, rating : {rating} """
        return prompt

    def get_conclusion_prompt(self):
        meta_description_prompt = f""" Get conclusion for the following keyword "{self.title}" """
