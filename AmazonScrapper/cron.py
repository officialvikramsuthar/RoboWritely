    ...
    def create_blogs(self):
        ...
        with jsonlines.open(file_path, mode='r') as reader, open(BASE_DIR + '/search_results_output.jsonl','r') as search_results_output:
            for data in reader:
                ...
                for keyword, product_data in data.items():

                    chatgpt.blog_gpt_chat()
                    prompt_obj = AmazonScrapperPrompats(keyword)
                    blog = []
                    ...
                    clean_response = clean_chat_gpt_response(title_response, trim_comma=True)
                    ...
                    clean_response = clean_chat_gpt_response(intro_response)
                    blog.append("<h2>Introduction</h2>")
                    blog.append("<p class='blog-intro'>"+clean_response+"</p>")
                    blog.append("</div>")

                    products_inserted = 0
                    for index, lines in enumerate(product_data):
                        ...
                        try:
                            ...
                            if product_in_seach_result and not rating:
                            ...
                            if name and description and price:
                                ...
                                product_response = clean_chat_gpt_response(product_response)
                                ...
                                if rating:
                                    ...
                                    rating = rating.replace("out of 5 stars", "").strip().split(" ")[0]
                                    if rating:
                    ...
                    clean_response = clean_chat_gpt_response(conlusion_response)
